...
function ScreenLoader({ isDark, label }: { isDark: boolean; label: string }) {
  return (
    <div className={`fixed inset-0 z-[100] flex items-center justify-center ${isDark ? "bg-slate-950/70" : "bg-slate-900/25"} backdrop-blur-sm`}>
      <div className={`flex min-w-[220px] flex-col items-center gap-3 rounded-[14px] border px-6 py-5 shadow-[0_18px_50px_rgba(15,23,42,0.22)] ${isDark ? "border-slate-700 bg-slate-900" : "border-[#dbe3ef] bg-white"}`}>
        <div className={`size-11 animate-spin rounded-full border-[3px] border-t-transparent ${isDark ? "border-slate-300 border-t-transparent" : "border-[#1f3a8a] border-t-transparent"}`} />
        <p className={`text-sm font-medium ${isDark ? "text-slate-100" : "text-[#0f172a]"}`}>{label}</p>
      </div>
    </div>
  );
}
...
export function AdminResultReviewDetailScreen({ submissionId, initialThemeDark = false }: AdminResultReviewDetailScreenProps) {
  ...
  const tabs = useMemo<{ id: ReviewTab; label: string }[]>(
    ...
    [hasAssessmentRows, hasCodingRows]
  );
  ...
  return (
    <main className={`min-h-screen ${isDark ? "bg-slate-950" : "bg-[#f8fafc]"}`}>
      {isSaving || isRefreshingAfterSave ? (
        <ScreenLoader
          isDark={isDark}
          label={isRefreshingAfterSave ? "Refreshing page..." : "Saving decision..."}
        />
      ) : null}
