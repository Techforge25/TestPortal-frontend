...
import { candidateLoginWithPasscode, getCandidateProfilePrefill } from "@/data.admin/shared/backendApi";
import { usePublicBranding } from "@/data.admin/shared/runtimeSettings";
...
function looksLikePakistaniAddress(value: string) {
  const normalized = String(value || "").trim().toLowerCase();
  if (!normalized) return false;
  return pakistanAddressKeywords.some((keyword) => normalized.includes(keyword));
}
...
export function CandidateRegistrationScreen() {
  ...
  const allowsGlobalAddress = useMemo(
    () =>
      isRemoteOrWorkFromHomeTest(
        `${loginDraft?.testTitle || ""} ${loginDraft?.testPosition || ""}`
      ),
    [loginDraft]
  );
...
function setValue(key: keyof FormState, value: string) {
  ...
  async function handleStartTest() {
    ...
    try {
      ...
      router.replace("/candidate/pre-test");
    } catch (submitError) {
