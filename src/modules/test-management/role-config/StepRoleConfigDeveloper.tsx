...
import type { CreateTestCodingTask } from "../types/createTest.types";

const languageOptions = ["JavaScript", "TypeScript", "Python", "Java", "C++", "Go", "PHP", "Ruby", "Dart"] as const;

type CodingTaskBlockProps = {
  title: string;
  taskName: string;
  description: string;
  marks: string;
  testCases: CreateTestCodingTask["testCases"];
  language: string;
...
type StepRoleConfigDeveloperProps = {
  step: number;
  isDark: boolean;
  codingTasks: CreateTestCodingTask[];
  setCodingTasks: Dispatch<SetStateAction<CreateTestCodingTask[]>>;
  roleCategory?: string;
};
...
function CodingTaskBlock({ title, taskName, description, marks, testCases, language, onTaskNameChange, onDescriptionChange, onTestCaseInputChange, onTestCaseOutputChange, onTestCaseWeightChange, onToggleTestCaseHidden, onAddTestCase, onDeleteTestCase, onLanguageChange, onDelete, isDark }: CodingTaskBlockProps) {
  return (
    <article className={`rounded-[10px] border p-3 ${isDark ? "border-slate-700 bg-slate-900" : "border-[#e2e8f0]"}`}>
      ...
      <div className="mt-3 flex items-center gap-2"><span className={`text-[30px] [zoom:0.5] ${isDark ? "text-slate-300" : "text-[#475569]"}`}>Marks:</span><span className={`inline-flex h-9 min-w-[96px] items-center justify-center rounded-[8px] border px-2 text-sm font-medium ${isDark ? "border-slate-600 bg-slate-800 text-slate-100" : "border-[#dbe3ef] bg-[#f8fafc] text-[#0f172a]"}`}>{marks}</span></div>
      <div className="mt-3 space-y-2">
        ...
        <div className="space-y-3">
          {testCases.map((testCase, idx: number) => (
...
export function StepRoleConfigDeveloper(props: StepRoleConfigDeveloperProps) {
  const { step, isDark, codingTasks, setCodingTasks } = props;
  if (step !== 3) return null;

  return (
    <CreateTestCard title="Add Coding Tasks" subtitle={`${codingTasks.length} Tasks Added • Marks auto-distributed from remaining total`} isDark={isDark}>
      <div className="flex justify-end">
        <AppButton
          variant="primary"
          size="md"
          leftIcon={<DiamondIcon />}
          onClick={() => setCodingTasks((prev) => [...prev, { id: prev.length + 1, title: `Task ${prev.length + 1}`, taskName: "New Task", language: "JavaScript", description: "Write the coding task statement here.", marks: "0", testCases: [{ id: 1, input: "[input] , expected", expectedOutput: "[output]", isHidden: false, weight: "1" }] }])}
      ...
      {codingTasks.map((task) => (
        <CodingTaskBlock
          ...
          onTaskNameChange={(value: string) => setCodingTasks((prev) => prev.map((item) => (item.id === task.id ? { ...item, taskName: value } : item)))}
          onLanguageChange={(value: string) => setCodingTasks((prev) => prev.map((item) => (item.id === task.id ? { ...item, language: value } : item)))}
          onDescriptionChange={(value: string) => setCodingTasks((prev) => prev.map((item) => (item.id === task.id ? { ...item, description: value } : item)))}
          onTestCaseInputChange={(caseId: number, value: string) => setCodingTasks((prev) => prev.map((item) => item.id === task.id ? { ...item, testCases: item.testCases.map((tc) => tc.id === caseId ? { ...tc, input: value } : tc) } : item))}
          onTestCaseOutputChange={(caseId: number, value: string) => setCodingTasks((prev) => prev.map((item) => item.id === task.id ? { ...item, testCases: item.testCases.map((tc) => tc.id === caseId ? { ...tc, expectedOutput: value } : tc) } : item))}
          onTestCaseWeightChange={(caseId: number, value: string) => setCodingTasks((prev) => prev.map((item) => item.id === task.id ? { ...item, testCases: item.testCases.map((tc) => tc.id === caseId ? { ...tc, weight: value || "1" } : tc) } : item))}
          onToggleTestCaseHidden={(caseId: number) => setCodingTasks((prev) => prev.map((item) => item.id === task.id ? { ...item, testCases: item.testCases.map((tc) => tc.id === caseId ? { ...tc, isHidden: !tc.isHidden } : tc) } : item))}
          onAddTestCase={() => setCodingTasks((prev) => prev.map((item) => item.id === task.id ? { ...item, testCases: [...item.testCases, { id: item.testCases.length > 0 ? Math.max(...item.testCases.map((tc) => tc.id)) + 1 : 1, input: "[input]", expectedOutput: "[output]", isHidden: true, weight: "1" }] } : item))}
          onDeleteTestCase={(caseId: number) => setCodingTasks((prev) => prev.map((item) => item.id === task.id ? { ...item, testCases: item.testCases.filter((tc) => tc.id !== caseId).length > 0 ? item.testCases.filter((tc) => tc.id !== caseId) : [{ id: 1, input: "[input]", expectedOutput: "[output]", isHidden: false, weight: "1" }] } : item))}
          onDelete={() => setCodingTasks((prev) => prev.filter((item) => item.id !== task.id).map((item, idx) => ({ ...item, id: idx + 1, title: `Task ${idx + 1}` })))}
          isDark={isDark}
        />
      ))}
    </CreateTestCard>
